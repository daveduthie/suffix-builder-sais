...
public class LCPArrayTest {
	...
	public void testLongestCommonPrefix() {
		String str = "AAA$";
		int skip = 0;
		int lcp = Tree.longestCommonPrefix(str, 2, 1, skip);
		assertTrue(lcp == 1);
	...
	public void testLongestCommonPrefixArray() {
		String str = "ABABDABC$";
		int[] suffixArray = SuffixArray.computeSA(str);
		int[] lcpArr = Tree.longestCommonPrefixArray(str, suffixArray);
		int[] expect = { 0, 2, 2, 0, 1, 1, 0, 0 };

		// for (int i = 0; i < suffixArray.length; ++i) {
		// int x = suffixArray[i];
		// SuffixArray.prn(i, x, (i < lcpArr.length) ? lcpArr[i] : "-", "->",
		// str.substring(x));
		// }
