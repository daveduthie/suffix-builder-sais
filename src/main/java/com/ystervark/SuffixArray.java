...
public class SuffixArray {
  ...
  public static int suffixCompare(String text, int[] suffixArray, String pattern, int pos,
      int skip) {
   ...
   * @return
   */
  public static int binarySearch(String text, int[] suffixArray, String pattern, int skip,
      boolean first) {
      ...
      if (cmp == 0) {
        if (first) {
          ...
          if (mid == suffixArray.length - 1
              || suffixCompare(text, suffixArray, pattern, mid + 1, skip) < 0) {
            // prn("gotcha:", mid);
            return mid;
          } else {
            lo = mid + 1;
  ...
  public static int[] longestCommonPrefixArray(String str, int[] order) {
    int[] lcpArray = new int[order.length];
    lcpArray[0] = 0;
    int lcp = 0;
  ...
  public static void SAIS(Text text, Text sa, Buckets buckets) {
    ...
    for (int i = sa.size() - 1; i >= 0; --i) {
      int curr = sa.get_at(i);
      if (curr > 0
          && (types[curr - 1] == SuffixType.ASCENDING || types[curr - 1] == SuffixType.VALLEY)) {
      ...
      for (int m : matches) {
        if (!expect.contains(m)) {
          throw new IllegalStateException("Broken:\ntext:" + text + "\npatt:" + patterns.toString()
              + "\nand " + m + " is in matches when it shouldn't be");
        } else {
          prn("++++++++++++++++++++++++++++++++++++++++++++++> match:", m);
        }
      }
